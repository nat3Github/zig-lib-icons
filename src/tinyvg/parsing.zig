// Vendored from zig-lib-svg2tvg, unchanged except for the import path.
...
pub fn Parser(comptime Reader: type) type {
    return struct {
        ...
        pub fn next(self: *Self) (Reader.Error || ParseError)!?DrawCommand {
            ...
            return switch (command) {
                ...
                .draw_lines => blk: {
                    ...
                    for (data.data.lines) |*ln| {
                        ln.start.x = try self.readUnit();
                        ln.start.y = try self.readUnit();
                        ln.end.x = try self.readUnit();
                        ln.end.y = try self.readUnit();
                    }
        ...
        fn readPath(self: *Self, path_length: usize) !Path {
            ...
            return Path{ .segments = buffers.first };
        }
